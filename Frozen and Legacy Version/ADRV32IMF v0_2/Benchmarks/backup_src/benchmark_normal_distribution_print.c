...
// calculates the factorial value of n, if returns 1 negative n is given 
...
int main()
{
    float sigma = 1.0, x = 0.0, mu = 0.0, result, expo, pi = 3.1415926535897932;

    expo = (x - mu) / sigma;
    result = (1.0 / (sigma * my_sqrt(2.0 * pi))) * eb_pow(-0.5 * expo * expo);

    printf("Ergebniss: f(x) = %f", result);
}
